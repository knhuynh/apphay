...
class AppController extends Controller {
    ...
    if(Input::hasFile('image')){
      ...
      $destinationPath = 'uploads/apps/' . Carbon::now()->format('Y/m/d'); // upload path
      if(!is_dir(public_path($destinationPath.'/thumbnails'))){
        mkdir(public_path($destinationPath.'/thumbnails'), 0777, true);
      }
      $filename = $app->slug.'.'.$image->getClientOriginalExtension();
      // Input::file('image')->move(public_path($destinationPath, $filename));
      Image::make($image->getRealPath())->save(public_path($destinationPath.'/'.$filename));
      Image::make($image->getRealPath())->resize(317,174)->save(public_path($destinationPath.'/thumbnails/'.$filename));
      $app->image = $destinationPath.'/'.$filename;
      $app->thumbnail = $destinationPath.'/thumbnails/'.$filename;
    }
